<!-- TITLE: Добро пожаловать в Saylat — с чего начать -->
<!-- CATEGORY: announcements -->

Привет! Это место для вопросов по Saylat, идей и настройке **личного сервера**.

## Быстрый старт

1. **APK** — [последний релиз](https://github.com/Baddysays/Saylat/releases/latest) или ваш сервер: `http://ВАШ_IP:8787/app/download/saylat.apk`
2. **Сервер** — скрипт `install-saylat-server.sh`, подробнее в [docs/LICHNYI-SERVER.md](https://github.com/Baddysays/Saylat/blob/main/docs/LICHNYI-SERVER.md)
3. В приложении: **Настройки → адрес сервера** (`http://IP:8787`)

## Как это выглядит

![Главная: режимы скорости](https://raw.githubusercontent.com/Baddysays/Saylat/main/docs/assets/screenshots/home-speed-modes.png)
![Настройки: чтение](https://raw.githubusercontent.com/Baddysays/Saylat/main/docs/assets/screenshots/settings-reading-modes.png)
![Настройки: темы](https://raw.githubusercontent.com/Baddysays/Saylat/main/docs/assets/screenshots/settings-themes-dark.png)

## Куда писать

| Тема | Куда |
|------|------|
| Вопрос «как настроить», сеть, VPS | [General](https://github.com/Baddysays/Saylat/discussions/new?category=general) |
| Идея функции | [Ideas](https://github.com/Baddysays/Saylat/discussions/new?category=ideas) |
| Вопрос с ответами сообщества | [Q&A](https://github.com/Baddysays/Saylat/discussions/new?category=q-a) |
| Ошибка в приложении / сервере | [Bug report](https://github.com/Baddysays/Saylat/issues/new?template=bug_report.md) |
| Быстрый контакт | [@baddysays](https://t.me/baddysays) |

## Не публикуйте

Пароли, `X-API-Key`, токены Telegram/VK, полные логи с секретами.

Спасибо, что пробуете Saylat 🥗
